...
- **Вертикальна «стіна»** (зроблено): лінії стоять фронтально — X=частоти (`WIDTH`), Y=час (`HEIGHT=9`, нові знизу→старі вгорі), Z=амплітуда (`h*AMP`, ріпли до глядача). Камера `position(-3.2,0,9.5)`, `lookAt(-3.2,0,0)`. (Раніше площина лежала горизонтально — амплітуда по Y, камера згори-збоку.)
