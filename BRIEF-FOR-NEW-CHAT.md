...
**⚠️ ВІДОМІ ПРОБЛЕМИ (треба виправити — головна задача):**
1. **Слайди не працюють.** Має бути **4 слайди, що зʼявляються один за одним при скролі** (як у baseone: пін + почергова поява текстів 01→02→03→04). Зараз видно лише перший, переходів немає. Структура `.gs-why-panel`×4 з пін-скролом і текстовими анімаціями (`splitWords` slide-up, blur/scale) **має лишитись** — користувач хоче зберегти ці анімації тексту.
2. **Анімація виглядає окремим «вікном»** — чорний прямокутник праворуч, відірваний від блоку. Має бути **фоном/частиною двох блоків** (текст + візуал), а не боксом. Маска країв (`mask-image`) не вирішила — бо блітимо у канвас на чорному фоні панелі. Потрібно або: зробити потік суцільним фоном секції під текстом, або коректно вписати в праву частину без видимих меж/чорного боксу.

**Бажана поведінка (зі слів користувача):**
- Дві колонки: ліворуч текст переваг, що **змінюється слайд-за-слайдом при скролі**; візуал-потік — спільний фон/права частина, **без боксу**.
- Текстові анімації оригіналу зберегти.
