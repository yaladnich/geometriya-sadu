...
## Глобальні дизайн-токени (незмінні правила)

**Border-radius: 2px (`0.125rem`) — на ВСІХ елементах сайту.**
Усі CSS-змінні виставлені в `0.125rem`: `--r-xs`, `--r-sm`, `--r-md`, `--r-lg`, `--r-xl`. Виняток — `--r-pill` (62rem, для хедера/пілюль). Ніколи не використовувати більші радіуси для карток, форм, модалів, FAQ-контейнера тощо.
