...
- Користувач хоче, щоб вона **стояла вертикально** («стіна» ліній обличчям до глядача): час по вертикалі (Y, нові знизу → старі вгорі), амплітуда — у глибину (Z, ріпли до глядача), камера фронтально.
- Реалізація: у створенні ліній `pos.y = (t/(HIST-1)-0.5)*ВИСОТА`, `pos.z = 0`; в `animate` амплітуду писати в `pos.z = h*AMP` (а не в `.y`); камера фронтально `position(-3.2, 0, ~9.5)`, `lookAt(-3.2, 0, 0)`.
